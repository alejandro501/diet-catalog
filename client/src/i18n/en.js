const en = {
  appTitle: 'Diet Catalog',
  dietNavigation: 'Diet navigation',
  foods: 'Foods',
  drinks: 'Drinks',
  smoothies: 'Smoothies',
  vitamins: 'Vitamins',
  spices: 'Spices',
  section: 'Section',
  filter: 'Filter',
  add: 'Add',
  remove: 'Remove',
  sectionExpand: 'Show all',
  sectionCollapse: 'Show less',
  items: 'items',
  language: 'Language',
  mobileMenu: 'Menu',
  mobileDietSelect: 'Diet type',
  noItems: 'No items match this filter.',
  loading: 'Loading your catalog...',
  saveError: 'Something went wrong while talking to Firebase.',
  missingConfig:
    'Firebase environment variables are missing. Add the VITE_FIREBASE_* values from your Firebase web app.',
  authTitle: 'Sign in to your catalog',
  authCopy: 'Use email and password to load the same catalog from any browser.',
  email: 'Email',
  password: 'Password',
  confirmPassword: 'Confirm Password',
  showPassword: 'Show',
  hidePassword: 'Hide',
  signIn: 'Sign In',
  signUp: 'Create Account',
  signOut: 'Sign Out',
  contactTitle: 'Contact',
  contactHeading: 'Contact',
  contactCopy: 'Send me a message if you feel like sharing an idea, a bug report, or anything else that pops into your mind.',
  contactName: 'Name',
  contactNamePlaceholder: 'Your name',
  contactReplyTo: 'Email',
  contactReplyToPlaceholder: 'you@example.com',
  contactSubject: 'Title',
  contactSubjectPlaceholder: 'What is this about?',
  contactMessage: 'Message',
  contactMessagePlaceholder: 'Write your message here.',
  contactMessageHelp: 'Message is required and must be between 20 and 1000 characters.',
  contactEmailInvalid: 'Enter a valid email address or leave it blank.',
  contactFieldTooLong: 'Name, email, and title must be 50 characters or fewer.',
  contactMessageInvalid: 'Message must be between 20 and 1000 characters.',
  contactSend: 'Send',
  authSuccessSignUp: 'Account created. You can now sign in.',
  passwordMismatch: 'Passwords do not match.',
  authInvalidEmail: 'Enter a valid email address.',
  authEmailInUse: 'That email is already registered.',
  authWeakPassword: 'Password must be at least 6 characters.',
  authInvalidCredential: 'Email or password is incorrect.',
  authTooManyRequests: 'Too many attempts. Try again later.',
  authOperationNotAllowed: 'Email/password sign-in is not enabled in Firebase Authentication.',
  authUnauthorizedDomain:
    'This domain is not authorized in Firebase Authentication. Add it under Authorized domains.',
  networkBlocked:
    'A browser extension or privacy tool is blocking Firebase requests. Disable blockers for this site and try again.',
  requestAborted: 'The request was interrupted before it finished. Try again.',
  addPlaceholder: {
    foods: 'Add a food',
    drinks: 'Add a drink',
    vitamins: 'Add a vitamin',
    spices: 'Add a spice',
  },
  filters: {
    All: 'All',
    Fruits: 'Fruits',
    Vegetables: 'Vegetables',
    'Side Dish': 'Side Dish',
    Protein: 'Protein',
    Grains: 'Grains',
    Water: 'Water',
    Tea: 'Tea',
    Juice: 'Juice',
    Smoothie: 'Smoothie',
    Coffee: 'Coffee',
    'Omega-3': 'Omega-3',
    Mineral: 'Mineral',
    Vitamin: 'Vitamin',
    Probiotic: 'Probiotic',
    Electrolyte: 'Electrolyte',
    Herb: 'Herb',
    Pepper: 'Pepper',
    Root: 'Root',
  },
  languages: {
    en: 'English',
    hu: 'Hungarian',
    es: 'Spanish',
    it: 'Italian',
  },
  menuCatalog: 'Catalog',
  menuProfile: 'Profile',
  menuUsers: 'Users',
  menuShared: 'Shared Lists',
  menuSettings: 'Settings',
  profileTitle: 'Profile',
  profileCopy: 'Set the public identity other users will see when browsing the app.',
  profileName: 'Name',
  profileUsername: 'Username',
  profilePublicLabel: 'Public profile',
  profilePublicHelp: 'If enabled, your profile appears in the Users directory and can be found for diet sharing.',
  profileSave: 'Save Profile',
  profileSaved: 'Profile saved.',
  profileNamePlaceholder: 'Your display name',
  profileUsernamePlaceholder: 'your-username',
  profileNameRequired: 'Name is required.',
  profileUsernameRequired: 'Username is required.',
  profileUsernameTaken: 'That username is already in use.',
  profileUsernameFormat: 'Username can only use letters, numbers, dots, underscores, and hyphens.',
  usersTitle: 'Users',
  usersCopy: 'Browse public profiles that can be found in the app.',
  usersEmpty: 'No users found yet.',
  sharedTitle: 'Shared Lists',
  sharedCopy: 'These are diet lists shared directly with you.',
  sharedEmpty: 'No lists have been shared with you yet.',
  sharedSelectPrompt: 'Choose a shared list to preview its catalog.',
  sharedLoading: 'Loading shared catalog...',
  sharedOwner: 'Shared by',
  shareWithUser: 'Share My List',
  unshareWithUser: 'Unshare My List',
  shareCreated: 'List shared.',
  shareRemoved: 'Shared access removed.',
  sharedWithYou: 'Shared with you',
  sharedByYou: 'You share with this user',
  anonymousUser: 'Anonymous User',
  settingsTitle: 'Settings',
  settingsCopy: 'Manage diet types and food categories used in your catalog.',
  settingsDietTypes: 'Diet Types',
  settingsFoodCategories: 'Food Categories',
  settingsAddDiet: 'Add Diet Type',
  settingsAddCategory: 'Add Food Category',
  settingsDietName: 'Diet Name',
  settingsDietNamePlaceholder: 'For example Paleo',
  settingsShowDiet: 'Show',
  settingsCategoryNamePlaceholder: 'For example Fermented',
  settingsDelete: 'Delete',
  settingsSave: 'Save Settings',
  settingsSaved: 'Settings saved.',
  settingsDietNameRequired: 'Diet name is required.',
  settingsDietNameTaken: 'That diet type already exists.',
  settingsCategoryNameRequired: 'Category name is required.',
  settingsCategoryNameTaken: 'That category already exists.',
  settingsNeedVisibleDiet: 'Keep at least one visible diet type.',
  smoothieNamePlaceholder: 'Add a smoothie name',
  smoothieNameLabel: 'Smoothie name',
  smoothieIngredients: 'Ingredients',
  smoothieIngredientPlaceholder: 'Add an ingredient',
  smoothieAddIngredient: 'Add Ingredient',
  smoothieSave: 'Add Smoothie',
  smoothieNoIngredients: 'No ingredients yet.',
  smoothieExpand: 'Open ingredients',
  smoothieCollapse: 'Close ingredients',
  dietShareGlobalLabel: 'Share globally',
  dietShareGlobalHelp: 'Anyone signed in can browse this diet list in Shared Lists.',
  dietShareSpecificLabel: 'Share with specific users',
  dietShareSearchPlaceholder: 'Search public users by name or username',
  dietShareNoSpecificUsers: 'No specific users added yet',
  dietShareGlobalEnabled: 'This diet list is now shared globally.',
  dietShareGlobalDisabled: 'Global sharing removed for this diet list.',
  dietShareRecipientAdded: 'User added to this shared diet list.',
  dietShareRecipientRemoved: 'User removed from this shared diet list.',
};

export default en;
